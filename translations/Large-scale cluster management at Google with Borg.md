...
Jobs有部署的约束，且需要处理少见的负载尖峰，我们的机器是异构的，我们从服务job回收资源来运行批处理job。因此为了评估我们的策略选择，我们需要一个比“平均利用率”更复杂的指标。在大量实验之后我们选择了 **cell压缩量** ：给定工作负载，我们通过不断移除机器直到当前的资源总量不足以支持工作负载，来确定一个cell需要的最小规模，从零开始重复部署此工作负载，确保我们不会被某个意料之外的配置所干扰。这提供了明确的终止条件，且促进了自动化程度，避免了生成和建模合成负载的陷阱。评估技术的定量比较可以在[78]查看：我们会发现细节令人惊讶地微秒。
...
在线上，我们特意为工作负载的增长，偶然的“黑天鹅实践”，负载尖峰，机器故障，硬件升级和大规模局部故障（如供电短路）保留了足够的裕度（headroom）。图4展示了如果我们执行cell合并（压缩），我们的实际cell可以有多小。图中使用这些压缩后的大小作为基准线。
...
很多其他组织在独立的集群里运行面向用户的job和批处理job，我们审视了一下如果我们也这么做会发生什么。图5显示，在一个中等规模的cell上，`prod job`和`non-prod job`隔离运行的话将多需要20%-30%的机器来支撑起工作负载。因为`prod jobs`通常会预留一些资源来应对罕见的负载尖峰，但大多数时间并不要这些资源。`Borg`会将这些资源回收来运行`non-prod jobs`，因此总体上我们并不需要那么多机器。
...
图11可以看到更多的细节，展示了预留部分与使用上限的比。一个超出内存限制的task，如果它的资源被其他task需要，它会首先被抢占，不管优先级有多高，所以极少有task会超过它的内存限制。另一方面，CPU很容易被限制，因此短期的尖峰会让使用量超出预留值，却不会有什么损害。
